...
const PasswordResetad = () => {
    ...
    const handleChangePassword = async () => {
        ...
        try {
            await axios.post('http://localhost:5012/api/change-password-admin', { email, password });
            setModalMessage('Password changed successfully.');
            setMessageType('success');
            setShowModal('true');
            setTimeout(() => navigate("/login-admin"), 5000);
        } catch (error) {
            console.error(error);
...
export default PasswordResetad;
